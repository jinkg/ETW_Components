...
package com.mybadlogic.gdx.scenes.scene2d.ui;
...
import com.mybadlogic.gdx.graphics.g2d.NinePatch;
...
import com.mybadlogic.gdx.utils.Json;
import com.mybadlogic.gdx.utils.ObjectMap;
import com.mybadlogic.gdx.utils.SerializationException;
...
public class Skin implements Disposable {
	...
	protected Json getJsonLoader (final FileHandle skinFile) {
		...
		class AliasSerializer implements Json.Serializer {
			...
			public void write (Json json, Object object, Class valueType) {
				for (ObjectMap.Entry<String, ?> entry : map.entries()) {
		...
		json.setSerializer(Skin.class, new Json.Serializer<Skin>() {
			public void write (Json json, Skin skin, Class valueType) {
				...
				for (ObjectMap.Entry<Class, ObjectMap<String, Object>> entry : data.resources.entries())
			...
			private void readTypeMap (Json json, ObjectMap<String, ObjectMap> typeToValueMap, boolean isResource) {
				if (typeToValueMap == null)
					throw new SerializationException("Skin file is missing a \"" + (isResource ? "resources" : "styles")
						+ "\" section.");
				for (ObjectMap.Entry<String, ObjectMap> typeEntry : typeToValueMap.entries()) {
					Class type;
					try {
					...
					for (ObjectMap.Entry<String, ObjectMap> valueEntry : valueMap.entries()) {
						try {
							if (isResource)
