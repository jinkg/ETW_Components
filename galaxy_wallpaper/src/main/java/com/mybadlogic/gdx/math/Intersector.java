...
import com.mybadlogic.gdx.math.collision.Ray;
...
public final class Intersector {
    ...
    public static boolean intersectRayPlane(Ray ray, Plane plane, Vector3 intersection) {
        ...
        if (denom != 0) {
            ...
            if (intersection != null)
                intersection.set(ray.origin).add(new Vector3(ray.direction).scl(t));
            return true;
    ...
    public static boolean intersectRaySphere(Ray ray, Vector3 center, float radius, Vector3 intersection) {
        ...
        float b = 2 * (dir.dot(new Vector3(start).sub(center)));
        ...
        if (t0 < 0) {
            if (intersection != null) intersection.set(start).add(new Vector3(dir).scl(t1));
            return true;
        }
        // else the intersection point is at t0
        else {
            if (intersection != null) intersection.set(start).add(new Vector3(dir).scl(t0));
            return true;
        }
    }
    ...
    public static boolean intersectRayTriangles(Ray ray, float[] triangles, Vector3 intersection) {
        ...
        for (int i = 0; i < triangles.length - 6; i += 9) {
            ...
            if (result == true) {
                float dist = new Vector3(ray.origin).sub(tmp).len();
                if (dist < min_dist) {
                    min_dist = dist;
                    best.set(tmp);
                    hit = true;
