...
public class CatmullRomSpline implements Serializable {
	...
	public List<Vector3> getPath (int numPoints) {
		...
		for (int i = 1; i <= controlPoints.size() - 3; i++) {
			...
			T1.set(controlPoints.get(i + 1)).sub(controlPoints.get(i - 1)).scl(0.5f);
			T2.set(controlPoints.get(i + 2)).sub(controlPoints.get(i)).scl(0.5f);
			...
			for (int j = 0; j < numPoints; j++) {
				...
				Vector3 point = new Vector3(controlPoints.get(i)).scl(h1);
				point.add(new Vector3(controlPoints.get(i + 1)).scl(h2));
				point.add(new Vector3(T1).scl(h3));
				point.add(new Vector3(T2).scl(h4));
				points.add(point);
				t += increment;
			}
		}
	...
	public void getPath (Vector3[] points, int numPoints) {
		...
		for (int i = 1; i <= controlPoints.size() - 3; i++) {
			...
			for (int j = 0; j < numPoints; j++) {
				...
				Vector3 point = points[idx++].set(controlPoints.get(i)).scl(h1);
				point.add(new Vector3(controlPoints.get(i + 1)).scl(h2));
				point.add(new Vector3(T1).scl(h3));
				point.add(new Vector3(T2).scl(h4));
				t += increment;
