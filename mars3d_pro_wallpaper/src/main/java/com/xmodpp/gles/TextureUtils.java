...
public class TextureUtils {
    public static boolean jni_ToETC1(InputStream input, OutputStream output, int downscale) {
        try {
            Options options = new Options();
            options.inSampleSize = downscale;
            options.inScaled = false;
            options.inPreferredConfig = Config.RGB_565;
            ETC1Util.writeTexture(jni_ToETC1(BitmapFactory.decodeStream(input, null, options)), output);
            output.flush();
            return true;
        } catch (Exception e) {
